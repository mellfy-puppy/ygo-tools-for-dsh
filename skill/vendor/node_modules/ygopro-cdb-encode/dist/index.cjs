...
// src/card-data-entry.ts
var STRING_SLOTS = 16;
var CardDataEntry = class extends import_ygopro_msg_encode.CardData {
  constructor() {
    super(...arguments);
    this.ot = 0;
    this.category = 0;
  ...
  fromSqljsRow(row) {
    ...
    let linkMarker = 0;
    if ((type & import_ygopro_msg_encode.OcgcoreCommonConstants.TYPE_LINK) >>> 0 !== 0) {
      linkMarker = defense;
      defense = 0;
    }
    ...
    if (code === 5405695) {
      ruleCode = alias;
      alias = 0;
    } else if (alias && !(type & import_ygopro_msg_encode.OcgcoreCommonConstants.TYPE_TOKEN)) {
  ...
  toSqljsRow() {
    ...
    const isLink = (type & import_ygopro_msg_encode.OcgcoreCommonConstants.TYPE_LINK) >>> 0 !== 0;
...
var TYPE_LINK = import_ygopro_msg_encode2.OcgcoreCommonConstants.TYPE_LINK >>> 0;
...
var YGOProCdb = class _YGOProCdb {
  ...
  addCard(card) {
    const cards = (0, import_nfkit.makeArray)(card);
  ...
  *step(...args) {
  ...
  resolveRuleCode(cards) {
    const needsResolve = cards.filter(
      (c) => !c.ruleCode && c.alias && !(c.type & import_ygopro_msg_encode3.OcgcoreCommonConstants.TYPE_TOKEN)
    );
    ...
    try {
      ...
      while (query.step()) {
        ...
        if (code === 5405695) {
          ruleCode = alias;
        } else if (alias && !(type & import_ygopro_msg_encode3.OcgcoreCommonConstants.TYPE_TOKEN)) {
          const CARD_ARTWORK_VERSIONS_OFFSET = 20;
          const isAlternative = alias && alias < code + CARD_ARTWORK_VERSIONS_OFFSET && code < alias + CARD_ARTWORK_VERSIONS_OFFSET;
          if (!isAlternative) {
            ruleCode = alias;

...
type TakeParam<S extends string, Acc extends string = ''> = S extends `${infer C}${infer Rest}` ? C extends ParamChar ? TakeParam<Rest, `${Acc}${C}`> : Acc : Acc;
type SkipParam<S extends string> = S extends `${infer C}${infer Rest}` ? C extends ParamChar ? SkipParam<Rest> : Rest : '';
type ExtractParams<S extends string> = S extends `${string}:${infer Rest}` ? TakeParam<Rest> | ExtractParams<SkipParam<Rest>> : never;
export type CdbSqljsParamKeys<S extends string> = ExtractParams<S>;
export type CdbSqljsParamsFromStmt<S extends string> = Exclude<ExtractParams<S>, keyof CdbSqljsRow> extends never ? ExtractParams<S> extends never ? {} : {
    [K in ExtractParams<S> & keyof CdbSqljsRow]: CdbSqljsRow[K];
} : never;
export type CdbFindOperatorName = 'Not' | 'LessThan' | 'MoreThan' | 'LessThanOrEqual' | 'MoreThanOrEqual' | 'And' | 'Or' | 'HasBit' | 'HasAllBits';
...
export type CdbFindFilter = {
    [K in keyof (CdbSqljsRow & CdbFindVirtualFields)]?: CdbFindValue<(CdbSqljsRow & CdbFindVirtualFields)[K]>;
};
export {};

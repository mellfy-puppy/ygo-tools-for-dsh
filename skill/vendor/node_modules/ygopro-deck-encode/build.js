#!/usr/bin/env node
/* build.js - node build.js [all|cjs|legacy|esm|types|clean] */
const fs = require('fs');
...
/* ------------------------- esbuild builds ------------------------- */
async function buildOne(kind, options) {
  ...
  const presets = {
    ...
    legacy: {
      format: 'cjs',
      outfile: path.join(DIST_DIR, 'index.legacy.js'),
      platform: 'node',
      target: 'node10',
    },
    esm: {
  ...
  if (!preset) {
    throw new Error(`[build] Unknown build kind: ${kind}`);
  }

  ensureDir(path.dirname(preset.outfile));
  console.log(`[build] ${kind} -> ${preset.outfile}`);
...
/* ------------------------- main dispatcher ------------------------- */
(async function main() {
  const sub = (process.argv[2] || 'all').toLowerCase(); // all | cjs | legacy | esm | types | clean
  ...
  switch (sub) {
    ...
    case 'legacy': {
      await buildOne('legacy', { external, tsconfig: tscPath, entryPoints });
      break;
    }
    case 'esm': {
